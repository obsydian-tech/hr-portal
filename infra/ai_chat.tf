...
resource "aws_lambda_function" "naleko_ai_chat" {
  ...
  environment {
    variables = {
      ...
      MODEL_SMART      = "global.anthropic.claude-sonnet-4-5-20251001-v1:0"
      # NH-81: JSON alias map — resolveModel() uses this for alias-based lookups
      MODEL_ALIASES             = jsonencode({ fast = "global.anthropic.claude-haiku-4-5-20251001-v1:0", smart = "global.anthropic.claude-sonnet-4-5-20251001-v1:0" })
