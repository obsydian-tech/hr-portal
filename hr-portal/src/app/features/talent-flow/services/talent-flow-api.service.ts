...
export class TalentFlowApiService {
  ...
  private handleError(error: unknown): Observable<never> {
    ...
    else if (err.status && err.status >= 500) userMessage = 'A server error occurred. Please try again later.';
...
export class TalentFlowApiService {
  ...
  getCandidates(filters?: PipelineFilters): Observable<PipelineResponse> {
    ...
    return this.http
      .get<PipelineResponse>(`${this.baseUrl}/candidates`, {
  ...
  createCandidate(payload: CreateCandidatePayload): Observable<{ candidateId: string }> {
    return this.http
      .post<{ candidateId: string }>(`${this.baseUrl}/candidates`, payload, {
        headers: this.authHeaders,
      })
      .pipe(catchError(this.handleError));
  }
  ...
  getConfig(configType: ConfigType, version?: string): Observable<ConfigResponse> {
    ...
    return this.http
      .get<ConfigResponse>(`${this.baseUrl}/config/${configType}`, {
        headers: this.authHeaders,
        params,
      })
      .pipe(catchError(this.handleError));
  ...
  updateConfig(configType: ConfigType, data: unknown): Observable<ConfigResponse> {
    return this.http
      .put<ConfigResponse>(`${this.baseUrl}/config/${configType}`, data, {
        headers: this.authHeaders,
      })
      .pipe(catchError(this.handleError));
  }
