/**
 * AI-003 (NH-129): talentFlowAuthorizer — HTTP API Lambda authorizer
 *
 * Validates the `x-api-key` request header against a secret stored in
 * AWS Secrets Manager (talent-flow/agent/api-key).
 *
 * During Secrets Manager key rotation the authorizer accepts BOTH the
 * AWSCURRENT and AWSPENDING versions, ensuring zero-downtime rotation
 * even when warm talentFlowAiChat containers still hold a cached
 * (old) key.
 *
 * Returns:
...
const logger = new Logger({ serviceName: 'talentFlowAuthorizer' });
...
// Cache a Set of valid keys (AWSCURRENT + AWSPENDING if in rotation).
let cachedKeys = null;
let cacheExpiry = 0;
...
async function getValidKeys() {
  ...
  const secretId = process.env.AGENT_API_KEY_SECRET_NAME ?? 'talent-flow/agent/api-key';
  const keys = new Set();
